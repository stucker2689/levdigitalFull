({
    ...
    setKanbanRecords : function(component, kanbanRecords, kanbanView){
        ...
        let defaultStatusHeaders = ['New', 'Assigned', 'In Process', 'On Hold'];
    ...
    changeViewHelper : function(component, event, helper, view){
        ...
        let statusViewCaseStatusList = ['New', 'Assigned', 'In Process', 'On Hold'];
        let sprintViewCaseStatusList = ['Assigned', 'In Process'];
        ...
        if(selectedMenuItem == 'Sprint Month Kanban'){
            ...
            component.set('v.allCaseStatusList', sprintViewCaseStatusList);
           ...
           let toDateYear = todaysDate.getFullYear();
            ...
            component.set('v.caseStatus', null);
            component.set('v.caseDept', null);
            component.set('v.allCaseStatusList', statusViewCaseStatusList);
    ...
    //handleEditRecordOnKanban : function(component, event, newEstimatedHours, newDueDate, newPrimDept, newSprintWeeks, newStatus, newRequirementsStatus, newQAStatus){
    handleEditRecordOnKanban : function(component, event, newEstimatedHours, newDueDate, newPrimDept, newSprintWeeks, newStatus, newContactObj, newContactId){
        let days = 86400000; //number of milliseconds in a day
        let isChanged = false;
        ...
        try{
            if(allKanbanRecs[parsedEditRecord.ColumnHeader][editRecPos]){
                if(!sprintView){
                    //Not in Ops Kanban
                    if(allKanbanRecs[parsedEditRecord.ColumnHeader][editRecPos].CaseId == parsedEditRecord.CaseId && (parsedEditRecord.EstimatedHours != newEstimatedHours || parsedEditRecord.DueDate != newDueDate || 
                        parsedEditRecord.PrimaryDepartment != newPrimDept || parsedEditRecord.SprintWeeks != newSprintWeeks || parsedEditRecord.Status != newStatus || parsedEditRecord.ContactId != newContactId)){
                        isChanged = true;
                        ...
                        if(parsedEditRecord.PrimaryDepartment != newPrimDept){
                            ...
                            if(!currentDepartmentList.includes(newPrimDept)){
                                ...
                                component.set('v.allCaseDeptList', currentDepartmentList);
                            }
                        ...
                        if(parsedEditRecord.ContactId != newContactId){
                            let newContact = new Object();
                            newContact.Id = newContactId;
                            newContact.Name = newContactObj.displayValue;
                            allKanbanRecs[parsedEditRecord.ColumnHeader][editRecPos].Contact = newContact;
                            allKanbanRecs[parsedEditRecord.ColumnHeader][editRecPos].ContactId = newContactId;
                        }
                                ...
                                for(let backupRec of allKanbanRecsBackup[header]){
                                    if(backupRec){
                                        if(backupRec.CaseId == parsedEditRecord.CaseId){
                                            ...
                                            if(parsedEditRecord.PrimaryDepartment != newPrimDept){
                                                console.log('Backup newPrimDept: ', newPrimDept);
                    ...
                    if(allKanbanRecs[parsedEditRecord.ColumnHeader][editRecPos].CaseId == parsedEditRecord.CaseId && (parsedEditRecord.EstimatedHours != newEstimatedHours || parsedEditRecord.DueDate != newDueDate || 
                        parsedEditRecord.PrimaryDepartment != newPrimDept || parsedEditRecord.SprintWeeks != newSprintWeeks)){
